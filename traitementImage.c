...
void *traitement(void *p) {
  ...
  for (int i = 1; i <= vPartage->nbZones; i++) {
    ...
    // Le premier traitement n'attend personne : il doit forcément finir sa zone
    // pour que les suivants la traite.
    if (args->idThread > 0) {
    ...
    printf(
        "[%i][%fs] Début du calcul sur la zone %i. Temps estimé: %i secondes\n",
        args->idThread, getTimeElapsed(vPartage->start), i, wait * 3);
    calcul(wait);
...
int main(int argc, char *argv[]) {
  ...
  sharedData.cond =
      (pthread_cond_t *)malloc(nTreatments * sizeof(pthread_cond_t));
  ...
  for (int i = 0; i < atoi(argv[1]); ++i) {
    ...
    if (pthread_create(&threads[i], NULL, traitement, (void *)&tabParams[i]) !=
        0) {
  ...
  printf("Thread principal : fin de tous les threads secondaires. Temps total "
         ": %fs\n",
